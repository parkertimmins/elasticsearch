...
public class ES87TSDBDocValuesProducer extends DocValuesProducer {
    ...
    public BinaryDocValues getBinary(FieldInfo field) throws IOException {
        ...
        return switch (entry.compression) {
            ...
            case COMPRESSED_WITH_FSST -> getCompressedBinaryFSST(entry);
            case COMPRESSED_WITH_LZ4 -> getCompressedBinaryLZ4(entry);
        };
    }
    ...
    private BinaryDocValues getCompressedBinaryFSST(BinaryEntry entry) throws IOException {
    ...
    private BinaryDocValues getCompressedBinaryLZ4(BinaryEntry entry) throws IOException {
        if (entry.docsWithFieldOffset == -1) {
            // dense
            final RandomAccessInput addressesData = this.data.randomAccessSlice(entry.addressesOffset, entry.addressesLength);
            final LongValues addresses = DirectMonotonicReader.getInstance(entry.addressesMeta, addressesData);
    ...
    private void readFields(IndexInput meta, FieldInfos infos, int version) throws IOException {
        for (int fieldNumber = meta.readInt(); fieldNumber != -1; fieldNumber = meta.readInt()) {
            ...
            if (type == ES87TSDBDocValuesFormat.NUMERIC) {
                ...
                binaries.put(info.name, readBinary(meta, version));
    ...
    private BinaryEntry readBinary(IndexInput meta, int version) throws IOException {
        ...
        if (version >= ES87TSDBDocValuesFormat.VERSION_BINARY_DV_COMPRESSION) {
        ...
        if (compression == BinaryDVCompressionMode.COMPRESSED_WITH_FSST) {
        ...
        } else if (compression == BinaryDVCompressionMode.COMPRESSED_WITH_LZ4){
