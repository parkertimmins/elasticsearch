...
public class CountSingle extends AggregateFunction implements ToAggregator, SurrogateExpression, AggregateMetricDoubleNativeSupport {
    public static final NamedWriteableRegistry.Entry ENTRY = new NamedWriteableRegistry.Entry(Expression.class, "CountSingle", CountSingle::new);
    ...
    public CountSingle(Source source, Expression field, Expression filter, Expression window) {
        super(source, field, filter, window, emptyList());
    }

    private CountSingle(StreamInput in) throws IOException {
        super(in);
    }
    ...
    @Override
    protected NodeInfo<CountSingle> info() {
        return NodeInfo.create(this, CountSingle::new, field(), filter(), window());
    }

    @Override
    public AggregateFunction withFilter(Expression filter) {
        return new CountSingle(source(), field(), filter, window());
    }

    @Override
    public CountSingle replaceChildren(List<Expression> newChildren) {
        return new CountSingle(source(), newChildren.get(0), newChildren.get(1), newChildren.get(2));
    }
    ...
    public AggregatorFunctionSupplier supplier() {
        ...
        return CountAggregatorFunction.supplierSingle();
    }
    ...
    public Expression surrogate() {
        ...
        if (field.foldable()) {
            ...
            return new Mul(
                ...
                new CountSingle(s, Literal.keyword(s, StringUtils.WILDCARD), filter(), window())
            );
        }
